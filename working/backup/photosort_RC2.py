...
V7.0 (Gold Master Refactor)
- Complete logical refactor for readability and long-term maintenance.
- Functions grouped by purpose (Core Utils, AI Modules, Workflows).
- Added camera-themed ASCII banner.
- All v7.0-RC2 bugfixes (config loader, --model override, --dry-run) are included.

V7.0 (AI Critic) Update:
- Adds '--critique' / '--art' command.
...
# New V7.0 Gold Master Banner - Inspired by user feedback
ASCII_BANNER = (
    f"\n"
    f"{R}{B}[ // PHOTO-ARCHIVE.SYS v7.0 // ]{E}\n"
    f"{C}[ STATUS: ONLINE // USER: NICK  ]{E}\n"
    f"{Y}+==================================================+{E}\n"
    f"{Y}|{E}   (O)                                            {Y}|{E}\n"
    f"{Y}|{E}  / | \\                                {G}[  ]{E}      {Y}|{E}\n"
    f"{Y}|{E} /_|_'\\   [JPG] [RW2] >> [PROCESS] >>   {G}[::]{E}      {Y}|{E}\n"
    f"{Y}|{E}  | | |                                {G}[::]{E}      {Y}|{E}\n"
    f"{Y}|{E} (SLR)    [CRITIC.AI] >> [ANALYZE] >>  {G}[__]{E}      {Y}|{E}\n"
    f"{Y}|{E}                                     {G}(VAULT){E}      {Y}|{E}\n"
    f"{Y}+==================================================+{E}"
)
...
# ==============================================================================
# VI. FEATURE WORKFLOWS (The "Tools")
# ==============================================================================
...
def critique_images_in_directory(directory: Path, dry_run: bool, APP_CONFIG: dict, max_workers: int = MAX_WORKERS):
    ...
    if "--model" in sys.argv:
        try:
            ...
            print(" ⚠️  Could not parse --model argument. Using default.")
...
def main():
    ...
    command_to_run = None
    # Stop 'sys.argv' from being processed if it's just --model
    cli_args = args - {"--model"} 
    if "--model" in args:
        try:
            model_index = sys.argv.index("--model") + 1
            if model_index < len(sys.argv):
                cli_args = cli_args - {sys.argv[model_index]}
        except Exception:
            pass # Will be handled in the function
