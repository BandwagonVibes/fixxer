...
def get_image_hash(image_path: Path) -> Optional[tuple[Path, imagehash.ImageHash]]:
    ...
    try:
        ...
        print(f"   ⚠️  Skipping hash for {image_path.name}: {e}")
        return image_path, None
# --- END NEW FUNCTION ---
...
def group_bursts_in_directory(directory: Path, dry_run: bool, max_workers: int = MAX_WORKERS):
    ...
    print(f"\n🎨 BakLLaVA Photo Organizer --- (Burst Stacker Mode)")
    ...
    print(f"\n📁 Found {len(all_burst_groups)} burst groups. Stacking them now...")
...
def main():
    ...
    if group_bursts:
        if not V5_LIBS_AVAILABLE:
            print("❌ FATAL: '--group-bursts' requires the 'imagehash' library.")
    ...
    print("\n🎨 BakLLaVA Photo Organizer --- (Ingest Mode)")
    ...
    if available_models:
        ...
        print("   ⚠️  No models found. Run 'ollama pull bakllava' to install one.")
