#!/usr/bin/env python3
"""
BakLLaVA Photo Organizer - GOLD MASTER V7.0
...
- Burst Stacking: Group similar shots, pick sharpest
- Quality Culling: Sort by sharpness and exposure
- EXIF Insights: Session analytics dashboard
- Smart Prep: Copy keepers to Lightroom folder

Tech Stack:
- Local AI via Ollama (bakllava, minicpm-v2.6)
- RAW support via dcraw (macOS)
- Config file: ~/.photosort.conf

Version History:
V7.0 - AI Critic feature, config file system, ASCII art integration
V6.5 - Auto mode refactor, workflow improvements
V6.4 - EXIF stats dashboard
V6.3 - Smart prep mode
V6.2 - Burst grouping improvements
...
# ==============================================================================
# II. ASCII ART & BANNER SYSTEM - VISIONCREW WAREZ EDITION
# ==============================================================================
...
def print_visioncrew_animated():
    ...
    - Text fade-in effect (no box, cleaner look)
    ...
    text_lines = [
        "PHOTOSORT v7.0 - AI Ingestion Engine",
    ...
    if COLORAMA_AVAILABLE:
        # =================================================================
        # PART 1: SCANLINE WIPEDOWN EFFECT
        # =================================================================
        # Simulate old CRT monitor scanlines drawing the ASCII art
        # Speed: ~0.08s per line = ~0.4s total (fast 90's computer feel)
        ...
        for line in vision_crew_art:
            ...
            time.sleep(0.08)  # Scanline speed - fast but visible
        
        print()  # Space after ASCII art
        
        # =================================================================
        # PART 2: TEXT FADE-IN (NO BOX)
        # =================================================================
        # Just show/hide text for flicker effect, then stable
        
        # Flicker sequence: show/hide/show/hide/show final
        flicker_states = [True, False, True, False, True]
        
        for show_text in flicker_states:
            # Move cursor up to overwrite previous text (if not first iteration)
            if flicker_states.index(show_text) > 0:
                sys.stdout.write(f"\033[{len(text_lines) + 1}A")
            
            if show_text:
                # Show text with red color
                for i, line in enumerate(text_lines):
                    if line == "":
                        print()
                    elif i < 2:
                        # First two lines: centered, bright red
                        print(f"{Fore.RED}{Style.BRIGHT}{line}{Style.RESET_ALL}".center(width))
                    else:
                        # Last three lines: left-aligned relative to center
                        # Center the block, but left-align text within it
                        left_pad = (width - 40) // 2  # Rough centering
                        print(f"{' ' * left_pad}{Fore.RED}{Style.BRIGHT}{line}{Style.RESET_ALL}")
            else:
                # "Ghost" lines for flicker
                for _ in range(len(text_lines)):
                    print()
            
            sys.stdout.flush()
            time.sleep(0.08)  # Flicker speed
        
        # =================================================================
        # PART 3: TYPE OUT BOTTOM LINES (one at a time)
        # =================================================================
        time.sleep(0.15)  # Pause before typing
        
        # Clear and redraw with typing effect
        clear_console()
        
        # Redraw ASCII art (instantly this time)
        for line in vision_crew_art:
            vision_part = line[:split_point]
            crew_part = line[split_point:]
            colored_line = f"{Fore.WHITE}{Style.BRIGHT}{vision_part}{Fore.RED}{Style.BRIGHT}{crew_part}{Style.RESET_ALL}"
        ...
        for i, line in enumerate(text_lines):
            if line == "":
                ...
                print(f"{' ' * left_pad}{Fore.RED}{Style.BRIGHT}{line}{Style.RESET_ALL}")
                time.sleep(0.12)  # Pause between typed lines
...
def show_banner(mode="quick"):
    ...
    - mode="animated": Full warez animation (for --auto, --critique)
    - mode="quick": Static banner (for other commands)
    """
    if mode == "animated" and COLORAMA_AVAILABLE:
        try:
            ...
            print(f"--- PHOTOSORT v7.0 --- (Animation failed: {e})")
            print_static_banner()
    else:
        print_static_banner()
...
def group_bursts_in_directory(directory: Path, dry_run: bool, APP_CONFIG: dict, max_workers: int = MAX_WORKERS):
    ...
    print(f"\n{'='*60}")
    print(f" 📸 BakLLaVA Photo Organizer --- (Burst Stacker Mode)")
    print(f"{'='*60}")
...
def prep_smart_export(directory: Path, dry_run: bool, APP_CONFIG: dict, max_workers: int = MAX_WORKERS):
    ...
    print(f"\n{'='*60}")
    print(f" ✨ BakLLaVA Photo Organizer --- (Smart Prep Mode)")
    print(f"{'='*60}")
...
def show_exif_insights(directory: Path, dry_run: bool, APP_CONFIG: dict, max_workers: int = MAX_WORKERS):
    ...
    print(f"\n{'='*60}")
    print(f" 📊 BakLLaVA Photo Organizer --- (EXIF Stats Mode)")
    print(f"{'='*60}")
...
def critique_images_in_directory(directory: Path, dry_run: bool, APP_CONFIG: dict, max_workers: int = MAX_WORKERS):
    ...
    print("\n" + "="*60)
    print(" 🎨 BakLLaVA Photo Organizer --- (AI Critic Mode)")
    print("="*60)
    ...
    if TQDM_AVAILABLE:
        pbar = tqdm(total=len(files_to_critique), desc=" 🎨 Critiquing", unit="img", ncols=80)
...
def auto_workflow(directory: Path, dry_run: bool, APP_CONFIG: dict, max_workers: int = MAX_WORKERS):
    ...
    if TQDM_AVAILABLE:
        pbar = tqdm(total=len(hero_files), desc=" 🤖 Archiving", unit="img", ncols=80)
...
def run_default_ingest(current_dir: Path, dry_run: bool, APP_CONFIG: dict):
    ...
    print(f"\n{'='*60}")
    print(f" 🤖 BakLLaVA Photo Organizer --- (Legacy Ingest Mode)")
    print(f"{'='*60}")
...
def main():
    ...
    # V7.0 GOLD: Updated dispatch table
    DISPATCH_TABLE = {
        ...
        '--group-bursts': (group_bursts_in_directory, V5_LIBS_AVAILABLE, V5_LIBS_MSG, "quick"),
        '-b': (group_bursts_in_directory, V5_LIBS_AVAILABLE, V5_LIBS_MSG, "quick"),
        '--cull': (cull_images_in_directory, V6_CULL_LIBS_AVAILABLE, V6_LIBS_MSG, "quick"),
        '-c': (cull_images_in_directory, V6_CULL_LIBS_AVAILABLE, V6_LIBS_MSG, "quick"),
        '--prep': (prep_smart_export, V6_CULL_LIBS_AVAILABLE, V6_LIBS_MSG, "quick"),
        '--pe': (prep_smart_export, V6_CULL_LIBS_AVAILABLE, V6_LIBS_MSG, "quick"),
        '--stats': (show_exif_insights, V6_4_EXIF_LIBS_AVAILABLE, V6_4_LIBS_MSG, "quick"),
        '--exif': (show_exif_insights, V6_4_EXIF_LIBS_AVAILABLE, V6_4_LIBS_MSG, "quick"),
    ...
    if command_to_run:
        ...
        if "--help" in args or "-h" in args:
            show_banner("quick")
            print("\n 🖼️  BakLLaVA Photo Organizer - Usage")
            ...
            if not COLORAMA_AVAILABLE:
                print(" Tip: Install colorama for animated banner: pip3 install colorama")
            return
